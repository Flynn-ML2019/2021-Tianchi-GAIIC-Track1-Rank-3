...
def train(model, train_loader, val_loader, fold):
    ...
    for epoch in range(run_config.separate_category_num_epochs):
        epoch += 1
        ...
        best_model_out_path = "%s/separate_category_fold_%d_best.pth" % (directory.MODEL_DIR, fold)
...
def k_fold(train_df):
    folds = StratifiedKFold(n_splits=run_config.n_splits, shuffle=True, random_state=2021).split(
        np.arange(train_df.shape[0]), train_df.category.values
    )
    ...
    for fold, (train_idx, val_idx) in enumerate(folds):
        # 每一折都要产生一个新模型
        model = NeZhaForSequenceClassification(BertModel.from_pretrained(directory.PRETRAIN_DIR), 12)

        model = model.to(run_config.device)

        workers = multiprocessing.cpu_count()

        train_dataset = CategoryTextDataset(train_df, train_idx)

        train_loader = DataLoader(
            train_dataset, batch_size=run_config.batch_size, shuffle=True, num_workers=workers, pin_memory=True
        )

        val_dataset = CategoryTextDataset(train_df, val_idx)

        val_loader = DataLoader(
            val_dataset, batch_size=run_config.batch_size, shuffle=False, num_workers=workers, pin_memory=True
        )
