...
def collect_type_path():
    ...
    for weight_path in all_weights_path:
        ...
        if method == 'separate' and task_name == 'category':
            weights_path_list.append(weight_path)

    return weights_path_list


def category_task_predict(all_test, sub_id):
    ...
    for weight_rel_path in tqdm(weights_path_list):
        ...
        category_task_pre.append(model_pre)

    return category_task_pre


def write_category_result(single_task_res, all_test, sub_id):
    """
    生成单个任务的预测文件
    """
    pres_all = []
    ...
    with open(directory.CATEGORY_RESULT_PATH, 'w') as f:
...
def main():
    ...
    category_res = category_task_predict(all_test, sub_id)
    write_category_result(category_res, all_test, sub_id)
