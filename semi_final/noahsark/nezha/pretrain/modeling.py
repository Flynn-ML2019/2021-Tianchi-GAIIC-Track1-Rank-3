...
from pretrain.gpu_environment import get_custom_getter
import collections
import copy
import json
import math
...
def _generate_relative_positions_embeddings(length, depth,
    ...
    embeddings_table = np.zeros([vocab_size,
                                 depth])  # range(vocab_size * depth)#tf.get_variable(name="embeddings", shape=[vocab_size, depth], initializer=create_initializer())
...
def transformer_model(input_tensor,
    ...
    tf.compat.v1.logging.info('use_relative_position: %s' % use_relative_position)
