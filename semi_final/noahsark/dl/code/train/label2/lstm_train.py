...
from lstm_datasets import TextDataset
...
def train_model(model, criterion, optimizer, lr_scheduler=None):
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        best_model_out_path = model_save_dir + "/" + 'LSTMfold_' + str(fold + 1) + '_best' + '.pth'
...
def val_model(model, criterion):
    ...
    log_loss = metrics.log_loss(labels_list, pres_list) / 12.0
    return log_loss
...
if __name__ == "__main__":
    ...
    '''
    # 去掉new_des中的停用词
    stopwords_path='./dl/code/train/label2/stopwords.txt'
    stopwords = []
    ...
    # 总共 条训练数据里面包含 条正常样本，这里的正常样本标签就使用[0,0,0,....,0,0]编码
    print('无异常样本:', train_df[train_df['label'] == ''].shape[0])  # 2622

    model_save_dir = './dl/user_data/model_data/label2'

    train_batch_size = LSTMConfig.train_batch_size
    val_batch_size = LSTMConfig.val_batch_size
    max_epoch = LSTMConfig.max_epoch
    embed_num = 859
    ...
    folds = StratifiedKFold(n_splits=LSTMConfig.n_splits, shuffle=True, random_state=2021).split(
    ...
    for fold, (trn_idx, val_idx) in enumerate(folds):
        ...
        model = BilstmAttention(embed_num)
        model.to(device)
        optimizer = torch.optim.AdamW(model.parameters(), lr=LSTMConfig.lr, weight_decay=5e-4)
        # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.2)
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=LSTMConfig.T_0,
                                                                            T_mult=LSTMConfig.T_mult, eta_min=1e-5,
                                                                            last_epoch=-1)
        ...
        val_loader = DataLoader(val_dataset,
                                ...
                                shuffle=False,
                                num_workers=4)
        val_loss = train_model(model, criterion, optimizer, lr_scheduler=lr_scheduler)
        kfold_best.append(val_loss)
